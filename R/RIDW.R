...
RIDW <- function(gauge,
  ...
  station<-gauge
  linear<-na.omit(ext@data) %>% tbl_df %>% mutate_all(as.numeric)
  base<-paste(sprintf("linear[[%s]]",2:nlayers(ext)),collapse ="+")
  formula<-as.formula(paste0("linear[[1]]~",base))
  llm <- lm(formula)
  ...
  OBSp <-sum(stack(mapply(function(i) cov[[i]]*llm$coefficients[i+1],1:nlayers(cov)))) + llm$coefficients[1]
  Ridw <- OBSp + mapa
  Ridw[Ridw < 0] <- 0
  # Save Data
  # ---------------------------------------------------------------
  list(Interpol=Ridw,params=list(bestp=bestparam,MSE=sqrt(mean(residual.best^2)),linear_Model=llm))
}
